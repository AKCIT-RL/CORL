# Disturbances for Augmenting Robot Trajectories implementation in JAX
...
class DARTConfig:
    ...
    # gradient updates during DART iterations
    dart_timesteps: int = int(1e5)
    # DART iterations
    dart_iterations: int = 10
    # DART data collection episodes per iteration
    dart_episodes: int = 100
    # number of parallel envs used only for DART data collection
    collect_num_envs: int = 8
    # DART noise estimation samples
    dart_samples_for_noise_estimation: int = 5000
...
def _resolve_expert_checkpoint_path(load_checkpoint_path: Optional[str]) -> Path:
   if not load_checkpoint_path:
      raise ValueError("--expert_checkpoint is required for DART test.")
...
def load_expert(config: DARTConfig) -> Callable[[np.ndarray], np.ndarray]:
...
def get_dataset(
    dataset, config: DARTConfig, clip_to_eps: bool = True, eps: float = 1e-5
) -> Tuple:
...
class DARTTrainState(NamedTuple):
    actor: TrainState
...
class DART(object):
    ...
    def update_actor(
        self,
        train_state: DARTTrainState,
        batch: Transition,
        rng: jax.Array,
        config: DARTConfig,
    ) -> Tuple["DARTTrainState", jnp.ndarray]:
...
def load_dart_train_state(
    ...
    return DARTTrainState(
        actor=actor_train_state,
    )    

def create_dart_train_state(
    rng: jax.Array,
    observations: jnp.ndarray,
    actions: jnp.ndarray,
    config: DARTConfig,
...
def get_actor_from_checkpoint(
    ...
    """
    Load a DART actor from a JAX checkpoint.
...
def evaluate(
    ...
    for episode in range(num_episodes):
        ...
        while not done and not truncated:
            ...
            if record_video and episode == num_episodes - 1:
                env.render()
...
def training_loop(
    ...
    obs_std,
    rng: jax.Array,
    index: int = 0, # for DART iterations, not used in initial training loop
):
...
def collect_data(
    ...
    obs_mean,
    obs_std,
    noise_fn: Callable[[jax.Array], np.ndarray],
    rng: jax.Array,
    record_video: bool = False,
) -> Transition:
    # Collect trajectories in parallel with expert_fn + noise
    observations = []
    actions = []
    rewards = []
    next_observations = []
    ...
    with tqdm.tqdm(total=config.dart_episodes, desc="Collecting DART data") as pbar:
        while collected_episodes < config.dart_episodes:
            ...
            while not np.all(done | truncated):
                ...
                noise_batch = []
                split_rngs = jax.random.split(noise_rng, num_envs)
                for env_rng in split_rngs:
                    noise_batch.append(np.asarray(noise_fn(env_rng)))
                noise_act = np.asarray(noise_batch, dtype=np.float32)
                if noise_act.ndim == 1:
                    noise_act = noise_act[None, ...]

                behavior_action = np.clip(expert_action + noise_act, -1.0, 1.0)

                next_obs, reward, step_done, step_truncated, _ = env.step(behavior_action)
                if record_video and collected_episodes + num_envs >= config.dart_episodes:
                    env.render()
            ...
            remaining = config.dart_episodes - collected_episodes
...
def expand_dataset(
    ...
    # Combine old dataset with new data collected from DART iterations
...
def estimate_noise(
    ...
    samples: int,
    rng: jax.Array,
)-> Callable[[jax.Array], np.ndarray]:
    action_dim = dataset.actions.shape[-1]
    covariance = np.zeros((action_dim, action_dim), dtype=np.float64)
    num_observations = len(dataset.observations)
    if num_observations == 0:
        raise ValueError("Cannot estimate noise from an empty dataset.")
    samples = max(1, samples)
    sample_indices = np.asarray(
        jax.random.randint(rng, shape=(samples,), minval=0, maxval=num_observations)
    )
    
    for idx in sample_indices:
        obs = dataset.observations[int(idx)]
        expert_action = dataset.actions[int(idx)]
        
        policy_action = policy_fn(obs)
        if policy_action.ndim > 1 and policy_action.shape[0] == 1:
            policy_action = policy_action[0]

        noise = expert_action.reshape(-1) - policy_action.reshape(-1)
        noise = np.asarray(noise, dtype=np.float64)
        covariance += np.outer(noise, noise)
    covariance /= float(samples)
    
    # Add small value for numerical stability
    covariance += 1e-6 * np.eye(action_dim)
    
    # Cholesky decomposition
    L = np.linalg.cholesky(covariance).astype(np.float32)
    L_jax = jnp.asarray(L)

    def noise_fn(rng: jax.Array) -> np.ndarray:
        z = jax.random.normal(rng, shape=(action_dim,))
        return np.asarray(L_jax @ z, dtype=np.float32)

    return noise_fn

@pyrallis.wrap()  # type: ignore
def train(config: DARTConfig):
    wandb.init(
    ...
    if config.load_model is not None:
        ...
        train_state = load_dart_train_state(example_batch.actions,config)
    else:
        train_state = create_dart_train_state(
    ...
    # DART iterations
    print("Starting DART iterations...")
    expert_fn = load_expert(config)

    for dart_iter in range(1, config.dart_iterations + 1):
        policy_fn = partial(act_fn, train_state)
        rng, noise_estimation_rng = jax.random.split(rng)
        noise_fn = estimate_noise(
            policy_fn,
            dataset,
            config.dart_samples_for_noise_estimation,
            noise_estimation_rng,
        )
        ...
        if config.video_dir is not None and config.dart_episodes > 0:
            video_path = os.path.join(config.video_dir, f"collect/{dart_iter}.mp4")
            collect_env.save_video(render_trajectory, save_path=video_path)
        
        dart_index = (config.initial_timesteps + (dart_iter - 1) * config.dart_timesteps) // config.n_jitted_updates
        
        train_state, rng = training_loop(
            config,
            update_fn,
            act_fn,
            config.dart_timesteps,
            train_state,
            dataset,
            env,
            obs_mean,
    ...
    if config.checkpoints_path is not None:
        num_steps = config.initial_timesteps + config.dart_iterations * config.dart_timesteps
        checkpoint = {
            "actor_params": flax.serialization.to_state_dict(train_state.actor.params),
            "obs_mean": np.array(obs_mean),
            "obs_std": np.array(obs_std),
