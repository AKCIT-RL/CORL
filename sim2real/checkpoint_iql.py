...
def get_actor_from_checkpoint(
    ...
    Returns:
        Dictionary with keys:
            - ``actor_fn``: JIT-compiled function (obs, seed, temperature -> actions).
            - ``get_action``: Wrapper that normalises obs and uses temperature=0.0.
    ...
    actor_model = GaussianPolicy(hidden_dims=hidden_dims, action_dim=action_dim)
    ...
    def actor_fn(
        ...
        return jnp.clip(dist.sample(seed=seed), -max_action, max_action)
    ...
    def get_action(
        ...
        return np.array(actor_fn(jnp.array(obs_normalized), seed=seed, temperature=temperature))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export an IQL checkpoint to a pickle file.")
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        required=True,
        help="Path to the IQL checkpoint directory.",
    )
    parser.add_argument(
        "--env-name",
        type=str,
